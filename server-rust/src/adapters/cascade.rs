...
impl LlmAdapter for CascadeAdapter {
    ...
    async fn generate_json_model(&self, model: &str, prompt: &str, schema: Option<JsonValue>, temperature: Option<f32>) -> Result<JsonValue> {
        ...
        let iter: Box<dyn Iterator<Item=&Box<super::AdapterDyn>> + '_> = if let Some(tp) = target_prov {
            Box::new(self.inner.iter().filter(move |a| a.provider_id() == tp))
        } else {
...
impl LlmAdapter for CascadeAdapter {
    ...
    async fn generate_json_model(&self, model: &str, prompt: &str, schema: Option<JsonValue>, temperature: Option<f32>) -> Result<JsonValue> {
        ...
        let mut last_err: Option<anyhow::Error> = None;
        for a in &self.inner {
            match a.generate_json_model(model, prompt, schema.clone(), temperature).await {
                Ok(v) => return Ok(v),
                Err(e) => { last_err = Some(e); }
            }
        }
    ...
    async fn generate_text_model(&self, model: &str, prompt: &str, system: Option<&str>, temperature: Option<f32>) -> Result<String> {
        ...
        let mut last_err: Option<anyhow::Error> = None;
        for a in &self.inner {
            match a.generate_text_model(model, prompt, system, temperature).await {
                Ok(v) => return Ok(v),
                Err(e) => { last_err = Some(e); }
            }
        }
